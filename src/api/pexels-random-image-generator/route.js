async function handler({
  ...
  try {
    console.log("Fetching from Pexels:", baseUrl); // デバッグ用ログ
    ...
    if (!response.ok) {
      ...
      return { error: "Failed to fetch from Pexels API" };
    }

    const data = await response.json();
    console.log("Pexels API raw response:", data); // デバッグ用ログ
    ...
    if (random) {
      ...
      console.log("Returning random photo:", result); // デバッグ用ログ
      return result;
    }
    ...
    console.log("Returning photo list:", result); // デバッグ用ログ
    return result;
  } catch (error) {
    console.error("Error in Pexels API handler:", error);
